...
def run_final_strategy_analysis(df, ticker):
    # [span_6](start_span)[span_7](start_span)[데이터 검증] 최소 30일 데이터 확인[span_6](end_span)[span_7](end_span)
    ...
    # [span_8](start_span)[span_9](start_span)향후 30일 예측 수행[span_8](end_span)[span_9](end_span)
...
if st.sidebar.button("전략 분석 실행"):
    try:
        with st.spinner('AI가 매매 시나리오를 설계 중입니다...'):
            ...
            # 1. [span_11](start_span)현재가 및 기간별 예측 수치[span_11](end_span)
            ...
            # 2. [span_12](start_span)[span_13](start_span)핵심 매매 가이드라인[span_12](end_span)[span_13](end_span)
            st.markdown("---")
            st.subheader("🎯 AI 추천 매매 가이드")
            c1, c2, c3 = st.columns(3)
            with c1:
                st.success(f"**적정 매수 가격**: {res['target_buy']:.2f}")
            with c2:
                st.error(f"**목표 매도 가격**: {res['target_sell']:.2f}")
            with c3:
                st.warning(f"**⚠️ 손절 가격**: {res['stop_loss']:.2f}")
...
st.markdown("---")
st.caption("면책 조항: 본 시스템은 통계적 모델 기반 예측치이며, 모든 투자 책임은 본인에게 있습니다.")
